...
void * tga_create( int width, int height, unsigned int format ) {
    ...
    case TGA_TrueCOLOR_32:
        return( (void *)mem_malloc( width * height * 4 ) );
        
    case TGA_TrueCOLOR_24:
        return( (void *)mem_malloc( width * height * 3 ) );
...
void * tga_load( const char * filename, 
    ...
    fread( &idlen, sizeof(ubyte), 1, targafile );
    fread( &cmap_type, sizeof(ubyte), 1, targafile );
    fread( &image_type, sizeof(ubyte), 1, targafile );
    //cmap_first         = ttohs( *(uint16 *)(&tga_hdr[HDR_CMAP_FIRST]) );
    File_ReadInt16( targafile, &cmap_first, 1 );
    //cmap_length        = ttohs( *(uint16 *)(&tga_hdr[HDR_CMAP_LENGTH]) );
    File_ReadInt16( targafile, &cmap_length, 1 );
    //cmap_entry_size    = (ubyte)tga_hdr[HDR_CMAP_ENTRY_SIZE];
    fread( &cmap_entry_size, sizeof(ubyte), 1, targafile );
    File_ReadInt16( targafile, &img_spec_xorig, 1 );
    File_ReadInt16( targafile, &img_spec_yorig, 1 );
    File_ReadInt16( targafile, &img_spec_width, 1 );
    File_ReadInt16( targafile, &img_spec_height, 1 );

    fread( &img_spec_pix_depth, sizeof(ubyte), 1, targafile );
    fread( &img_spec_img_desc, sizeof(ubyte), 1, targafile );
   ...
   // free( tga_hdr );
    ...
    if( cmap_type ) {
        ...
        colormap = (ubyte *)mem_malloc( cmap_bytes );
    ...
    image_data = (ubyte *)mem_malloc( bytes_total );
...
int tga_write_rle( const char * file, int width, int height, unsigned char * dat, unsigned int format ) {
    ...
    unsigned char * rawbuf = (unsigned char *)mem_malloc( width * format );  
