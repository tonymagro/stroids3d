...
char	*NET_AdrToString (netadr_t a)
{
	...
	if (a.type == NA_LOOPBACK)
		sprintf (s, sizeof(s), "loopback");
	else if (a.type == NA_IP)
		sprintf (s, sizeof(s), "%i.%i.%i.%i:%i", a.ip[0], a.ip[1], a.ip[2], a.ip[3], ntohs(a.port));
	else
		sprintf (s, sizeof(s), "%02x%02x%02x%02x:%02x%02x%02x%02x%02x%02x:%i", a.ipx[0], a.ipx[1], a.ipx[2], a.ipx[3], a.ipx[4], a.ipx[5], a.ipx[6], a.ipx[7], a.ipx[8], a.ipx[9], ntohs(a.port));
		...
		if (ret == -1)
		{
			...
			//if (dedicated->value)	// let dedicated servers continue after errors
				printf ("NET_GetPacket: %s from %s\n", NET_ErrorString(),
						NET_AdrToString(*net_from));
			//else
			//	OS_Error (ERR_DROP, "NET_GetPacket: %s from %s", 
...
void NET_SendPacket (netsrc_t sock, int length, void *data, netadr_t to)
{
	...
	if (ret == -1)
	{
		...
		*/
			...
			{
				printf ("NET_SendPacket Warning: %s : %s\n", 
						NET_ErrorString(), NET_AdrToString (to));
			}
			else
			{
				OS_Error( ERROR_DROP, va("NET_SendPacket ERROR: %s to %s\n", 
						NET_ErrorString(), NET_AdrToString (to)) );
			}
		//}
	}
...
int NET_IPSocket (char *net_interface, int port)
{
	...
	if ((newsocket = socket (PF_INET, SOCK_DGRAM, IPPROTO_UDP)) == -1)
	{
		...
		if (err != WSAEAFNOSUPPORT)
			printf ("WARNING: UDP_OpenSocket: socket: %s", NET_ErrorString());
		return 0;
	}
	...
	{
		printf ("WARNING: UDP_OpenSocket: setsockopt SO_BROADCAST: %s\n", NET_ErrorString());
		return 0;
	}
	...
	{
		printf ("WARNING: UDP_OpenSocket: bind: %s\n", NET_ErrorString());
...
void NET_OpenIP (void)
{
	...
	{
		port = Cvar_Get("ip_hostport", "0", 0)->value;
		if (!port)
		{
			port = Cvar_Get("hostport", "0", 0)->value;
			if (!port)
			{
				port = Cvar_Get("port", va("%i", PORT_SERVER), 0)->value;
			}
		}
		ip_sockets[NS_SERVER] = NET_IPSocket (ip->string, port);
		if (!ip_sockets[NS_SERVER] && dedicated)
			OS_Error (ERROR_FATAL, "Couldn't allocate dedicated server IP port");
	}
	...
	{
		port = Cvar_Get("ip_clientport", "0", 0)->value;
		if (!port)
		{
			port = Cvar_Get("clientport", va("%i", PORT_CLIENT), 0)->value;
			if (!port)
				port = PORT_ANY;
		}
...
int NET_IPXSocket (int port)
{
	...
	if ((newsocket = socket (PF_IPX, SOCK_DGRAM, NSPROTO_IPX)) == -1)
	{
		err = WSAGetLastError();
		if (err != WSAEAFNOSUPPORT)
			printf ("WARNING: IPX_Socket: socket: %s\n", NET_ErrorString());
		return 0;
	}
	...
	if (ioctlsocket (newsocket, FIONBIO, &_True) == -1)
	{
		printf ("WARNING: IPX_Socket: ioctl FIONBIO: %s\n", NET_ErrorString());
		return 0;
	}
	...
	{
		printf ("WARNING: IPX_Socket: setsockopt SO_BROADCAST: %s\n", NET_ErrorString());
		return 0;
	}
	...
	if( bind (newsocket, (void *)&address, sizeof(address)) == -1)
	{
		printf ("WARNING: IPX_Socket: bind: %s\n", NET_ErrorString());
		closesocket (newsocket);
		return 0;
	}
...
void NET_OpenIPX (void)
{
	...
	{
		port = Cvar_Get("ipx_hostport", "0", 0)->value;
		if (!port)
		{
			port = Cvar_Get("hostport", "0", 0)->value;
			if (!port)
	...
	{
		port = Cvar_Get("ipx_clientport", "0", 0)->value;
		if (!port)
		{
			port = Cvar_Get("clientport", va("%i", PORT_CLIENT), 0)->value;
			if (!port)
...
void NET_Init (void)
{
	...
	if (r)
		OS_Error (ERROR_FATAL,"Winsock initialization failed.");

	printf("Winsock Initialized\n");

	noudp = Cvar_Get ("noudp", "0", 0);
	noipx = Cvar_Get ("noipx", "0", 0);
